...
export default function ReportsClient() {
  ...
  return (
    <div className="space-y-6 p-6">
      {error && <p className="rounded-2xl border border-rose-300/20 bg-rose-500/10 p-5 text-sm text-rose-200">Could not load report data: {error}</p>}

      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0b0d1d] p-7 text-white shadow-2xl sm:p-9">
        <div className="pointer-events-none absolute -right-20 -top-32 h-80 w-80 rounded-full bg-violet-600/30 blur-3xl" />
        <div className="relative max-w-2xl">
          <span className="inline-flex rounded-full border border-cyan-300/20 bg-blue-500/10 px-3 py-1 text-xs font-semibold text-blue-200">Report center</span>
      ...
      <div>
        {!data && (
          <section className="mb-7 rounded-2xl border border-white/10 tm-glass p-8 text-center shadow-[0_12px_40px_rgba(15,23,42,0.07)] backdrop-blur-xl">
            <FileText className="mx-auto text-cyan-300" size={32} />
            <h2 className="mt-4 text-xl font-bold text-white">No report data yet</h2>
            <p className="mt-2 text-sm text-slate-400">Upload orders first.</p>
          </section>
        )}
        {data && (
          <>
        <section className="mb-7 rounded-2xl border border-white/10 tm-glass p-6 shadow-[0_12px_40px_rgba(15,23,42,0.07)] backdrop-blur-xl">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">AI Summary Preview</p>
              <h2 className="mt-2 text-lg font-bold text-white">Smart Action Plan</h2>
            </div>
            <span className="rounded-2xl bg-blue-500/10 p-2 text-cyan-300"><Sparkles size={20} /></span>
          </div>
          {hasActionPlan ? (
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              <div className="rounded-2xl bg-blue-500/10 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-blue-200">Overall Health</p>
                <p className="mt-1 font-bold text-blue-100">{smartSuggestions.overall_health}</p>
              </div>
              <div className="rounded-2xl bg-white/[0.06] p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Priority Actions</p>
                <p className="mt-1 text-sm text-slate-300">{smartSuggestions.priority_actions?.[0]}</p>
              </div>
              <div className="rounded-2xl bg-rose-500/10 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-rose-200">Risk Plan</p>
                <p className="mt-1 text-sm text-rose-100">{smartSuggestions.risk_suggestions?.[0]}</p>
              </div>
              <div className="rounded-2xl bg-emerald-500/10 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-emerald-200">Profit Plan</p>
                <p className="mt-1 text-sm text-emerald-100">{smartSuggestions.profit_suggestions?.[0]}</p>
              </div>
              <div className="rounded-2xl bg-violet-500/10 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-violet-200">Stock Plan</p>
                <p className="mt-1 text-sm text-violet-100">{smartSuggestions.stock_suggestions?.[0]}</p>
              </div>
            ...
            <p className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-500/10 p-4 text-sm font-medium text-amber-100">Upload or re-analyze orders to generate AI action plan.</p>
          )}
        </section>

        <div className="mb-4">
          <h2 className="text-lg font-bold text-white">Available report downloads</h2>
          <p className="mt-1 text-sm text-slate-400">Data source: {dataSource} · Last generated timestamp: {generatedAt.toLocaleString()}</p>
        </div>
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {reports.map(({ title, description, icon: Icon, tone, build, filename }) => (
            <article key={title} className="group overflow-hidden rounded-2xl border border-white/10 tm-glass p-6 shadow-[0_12px_40px_rgba(15,23,42,0.07)] backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-xl">
              ...
              <button type="button" disabled={!data} onClick={() => downloadCsv(filename, build())} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:bg-white/[0.12] disabled:text-slate-400">
                <FileDown size={17} /> Download CSV
              </button>
            </article>
          ))}
          <article className="overflow-hidden rounded-2xl border border-dashed border-white/20 bg-white/[0.05] p-6 opacity-75">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/[0.08] text-slate-400"><FileText size={23} /></div>
            <h3 className="mt-6 text-lg font-bold text-slate-300">PDF Executive Report</h3>
            <p className="mt-2 min-h-16 text-sm leading-6 text-slate-400">Polished PDF export for stakeholder review.</p>
            <button type="button" disabled className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-white/[0.12] px-4 py-3 text-sm font-semibold text-slate-400">
              Coming soon
            </button>
          </article>
        </div>
