...
function LoginContent() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#070816] px-5 py-6 text-white sm:px-8 lg:flex lg:items-center lg:py-10">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-6xl overflow-hidden rounded-2xl border border-white/10 bg-white/[0.055] shadow-[0_35px_100px_rgba(0,0,0,0.45)] backdrop-blur-xl lg:grid-cols-[0.94fr_1.06fr]">
        <section className="p-7 sm:p-10 lg:p-12">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-blue-500/10 px-3 py-1.5 text-xs font-semibold text-blue-200"><Sparkles size={14} /> Secure workspace access</span>
          ...
          {step === "email" ? (
            <form onSubmit={handleRequestOtp} className="mt-7 space-y-5">
              <div><label className="mb-2 block text-sm font-medium text-slate-200" htmlFor="email">Email address</label><div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 transition focus-within:border-cyan-300/70 focus-within:ring-4 focus-within:ring-blue-500/10"><Mail size={18} className="text-slate-400" /><input id="email" type="email" required autoFocus autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="w-full bg-transparent py-4 text-sm outline-none placeholder:text-slate-300" /></div></div>
